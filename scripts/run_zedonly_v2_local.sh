#!/usr/bin/env bash
# Local (8xH100) run of the ZED-only v2 policy — replaces cancelled cloud job 13.
# Chain: full abc v2 dataset sync (with retries; the SZ<->S3 link is flaky) ->
# per-config norm stats (ZED-subset via the config's station filter) -> train
...
CONFIG=pi05_siemens_packing_abcloader_v2_zedonly_bs128
EXP=siemens_packing_pi05_zedonly_v2_20260826
...
log() { echo "[zedonly-local $(date -u +%Y-%m-%dT%H:%M:%SZ)] $*"; }
...
if [ ! -s "$NS_FILE" ] || [ "$(stat -c%s "$NS_FILE")" -lt 200 ]; then
    log "2/4 computing norm stats (50k frames, ZED subset via config filter)"
