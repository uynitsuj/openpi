...
def select_keyframes_helper(head_trajectory: np.ndarray, head_camera_fov: float, past_idxs: deque, criterion: str = "cost_select") -> List[int]:
    ...
    if criterion == "cost_select":
        ...
        if len(past_idxs) > 0:
            ...
            substantial_angle_delta = angles_all[-1] > head_camera_fov * 0.2
            substantial_dist_delta = dists_all[-1] > 0.2
...
def cost_select(head_trajectory: np.ndarray, lookback_time_s: float = 2.0, timestep_weight: float = 0.01, angle_weight: float = 30.0, velocity_weight: float = 2500.0) -> List[int]:
    """
    ...
    thetas = [np.arccos(np.dot(z_vec, current_head_z_vec) / (np.linalg.norm(z_vec) * np.linalg.norm(current_head_z_vec))) for z_vec in previous_head_z_vecs]
    theta_costs = [(1/theta)*angle_weight for theta in thetas]
    ...
    delta_thetas = [np.arccos(np.dot(head_quats.as_matrix()[i-1, 2], head_quats.as_matrix()[i, 2]) / (np.linalg.norm(head_quats.as_matrix()[i-1, 2]) * np.linalg.norm(head_quats.as_matrix()[i, 2]))) for i in range(1, len(head_quats.wxyz))]
