...
- episodes the operator marked as failures (`eval_anno.json` scores.success,
  gello_buttons annotation) are excluded entirely by default
  (`--failed-episodes exclude`, per 2026-09-11 decision); pass
  `--failed-episodes teleop_only` to instead retain their teleop correction
  segments (the ABC/HG-DAgger-style alternative);
- policy segments are approved only for operator-marked successes, so
...
def scan_episode(episode: Path, reviewer: str, failed_episodes: str) -> dict | None:
    ...
    if not success and failed_episodes == "exclude":
        return skip("operator marked episode failed (--failed-episodes exclude)")
...
def main() -> None:
    ...
    parser.add_argument(
        "--failed-episodes", choices=("exclude", "teleop_only"), default="exclude",
        help="exclude: drop operator-marked failures entirely (default); "
        "teleop_only: retain their teleop correction segments (ABC/HG-DAgger style)",
    )
    ...
    for spec in args.collection:
        ...
        for episode in episodes:
            entry = scan_episode(episode, args.reviewer, args.failed_episodes)
    ...
    manifest = {
        ...
        "policy_note": (
            f"Generated by build_market42_review.py (failed_episodes={args.failed_episodes}): episode-level "
            "batch review, NOT interval-level human scrubbing. Operator-marked failures are "
            + ("excluded entirely" if args.failed_episodes == "exclude" else "included for teleop segments only")
            + "; policy segments approved only for successes. assumed_command_ramp_s measured from 288 "
            "handoffs on 2026-09-11 (median settle 2ms, max 1.33s); recordings are subscriber-driven and "
            "predate recorder ramp/sync archiving."
        ),
        "episodes": entries,
    }
