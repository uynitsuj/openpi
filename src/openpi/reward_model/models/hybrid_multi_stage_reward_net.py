...
class RewardTransformer(nn.Module):
    ...
    def forward(self,
        ...
        # Concatenate streams (N cameras + lang + state) → (B, N+2, T, D)
        x = torch.cat([vis_proj, lang_proj, state_proj], dim=1)

        # Tag first visual frame for each camera
        x[:, :N, 0, :] += self.first_pos

        # Flatten to tokens for Transformer: (B, (N+2)*T, D)
        x_tok = x.view(B, (N + 2) * T, D)

        # Build key padding mask from lengths, broadcast to all streams
        ...
        # Encode
        h = self.transformer(x_tok, src_key_padding_mask=mask)      # (B, (N+2)*T, D)

        # Back to (B, T, (N+2)*D), fuse to (B, T, D)
