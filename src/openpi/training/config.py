...
_CONFIGS = [
    ...
    # v3 of the simple-D405 job (2026-09-02): full refresh at 3058 episodes —
    # quality good or not-yet-labeled only (7 poor dropped), duration > 20s
    # (8 dropped). Standard pad+resize pipeline (resize_with_pad to 224, NO
    # center crop) — serving is the plain letterbox path, same as v1/v2.
    TrainConfig(
        name="pi05_siemens_simple_d405_v3_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
            repo_id="siemens_simple_d405_v3",
            default_prompt="industrial packing",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 3058,  # ~10 held-out episodes
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    # Curated last-24h slice (built 2026-09-03 05:10 CST): 945 eps, all
    # sz_44/20260902, same filters/trim as v4. Primarily for norm stats +
    # freshest-data fine-tunes; swap weight_loader to a v4 checkpoint before
    # using it that way.
    TrainConfig(
        name="pi05_siemens_simple_d405_24h_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
            repo_id="siemens_simple_d405_24h",
            default_prompt="industrial packing",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 945,  # ~10 held-out episodes
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    # v7dj_recent: same recent-window date cutoff (>= 2026-09-02 02:50Z = Sep 1
    # 19:50 PDT) + >20s + exclude explicitly-bad, refreshed with the latest data
    # (2670 eps vs v6dj_recent's 1353). driver joint order, baked packing prompt,
    # from pi05_base only.
    TrainConfig(
        name="pi05_siemens_simple_d405_v7dj_recent_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
            repo_id="siemens_simple_d405_v7dj_recent",
            default_prompt="Pack one transparent bag into the cardboard box and flatten the bag.",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 2376,  # 2670 CSV eps -> 2376 converted (duration/MCAP recheck)
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    # v8dj generation (2026-09-07): fresh query of the same job with ~3 more days
    # of collection. Same filters/lineage as v6dj/v7dj (completed, >20s, exclude
    # explicitly-bad, driver joint order via --no-flip-joints, baked packing
    # prompt). Two pools from one query (data through 2026-09-06 02:01Z):
    #   full  : 7376 CSV eps (v6dj was 4232, +3144)
    #   recent: 4492 CSV eps, created>=2026-09-02 02:50Z (v7dj_recent was 2670, +1822)
    # From pi05_base ONLY; never mix with flip-lineage v1-v4. 20k steps.
    TrainConfig(
        name="pi05_siemens_simple_d405_v8dj_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
            repo_id="siemens_simple_d405_v8dj",
            default_prompt="Pack one transparent bag into the cardboard box and flatten the bag.",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 7209,  # 7376 CSV eps -> 7209 converted (MCAP recheck)
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    TrainConfig(
        name="pi05_siemens_simple_d405_v8dj_recent_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
            repo_id="siemens_simple_d405_v8dj_recent",
            default_prompt="Pack one transparent bag into the cardboard box and flatten the bag.",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 4314,  # 4492 CSV eps -> 4314 converted (MCAP recheck)
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    TrainConfig(
        ...
        data=LeRobotYamRormDataConfig(
            ...
            default_prompt="Pack one transparent bag into the cardboard box and flatten the bag.",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 8603,  # 9148 CSV eps -> 8603 converted (MCAP recheck)
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    TrainConfig(
        ...
        data=LeRobotYamRormDataConfig(
            ...
            default_prompt="Pack one transparent bag into the cardboard box and flatten the bag.",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 5710,  # 6264 CSV eps -> 5710 converted (MCAP recheck)
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    # v10dj generation (2026-09-08): freshest-query pair, same filters/lineage as
    # v6-v9 (completed, >20s, exclude explicitly-bad, --no-flip-joints driver order,
    # baked packing prompt, from pi05_base). Data through 2026-09-08 11:01Z:
    #   full  : 9305 CSV eps (only +157 vs v9 — DB processing lags ~3h behind wall-clock)
    #   recent: 6421 CSV eps, created>=2026-09-02 02:50Z (+157 vs v9)
    # From pi05_base ONLY; never mix with flip-lineage v1-v4. 20k steps.
    TrainConfig(
        name="pi05_siemens_simple_d405_v10dj_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
    ...
    TrainConfig(
        name="pi05_siemens_simple_d405_v10dj_recent_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
    ...
    # v11dj generation (2026-09-09): NEW LINEAGE vs v6-v10. Two changes baked at
    # conversion time (convert_xdof_mcap_job.py @ 0556d2c):
    #   1) actions = recorded LEADER stream (action-*.mcap), not the follower state copy.
    #   2) top camera CENTER-CROPPED to its largest square (--resize-mode top_center_crop);
    #      wrists stay letterbox-padded. All 224x224.
    # Still driver joint order (--no-flip-joints -> straight-through serving, no flip)
    # + baked packing prompt, from pi05_base. SERVING must center-crop the top view to
    # match, and this lineage's norm stats differ (leader-action distribution) — never
    # mix with v6-v10 (follower/pad) or flip-lineage v1-v4 checkpoints/data.
    # Fresh query, data through 2026-09-09 08:16Z:
    #   full  : 10605 CSV eps (v10 was 9305, +1300)
    #   recent: 7721 CSV eps, created>=2026-09-02 02:50Z (v10 was 6421, +1300)
    # 20k steps.
    TrainConfig(
        name="pi05_siemens_simple_d405_v11dj_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
            repo_id="siemens_simple_d405_v11dj",
            default_prompt="Pack one transparent bag into the cardboard box and flatten the bag.",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 9545,  # 10605 CSV eps -> 9545 converted (de-prod upload lag + MCAP recheck)
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    TrainConfig(
        name="pi05_siemens_simple_d405_v11dj_recent_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
            repo_id="siemens_simple_d405_v11dj_recent",
            default_prompt="Pack one transparent bag into the cardboard box and flatten the bag.",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 6651,  # 7721 CSV eps -> 6651 converted (de-prod upload lag; leader-actions lineage)
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    # v13short25{cc,tc}f (2026-09-11): IDENTICAL datasets to the non-f variants,
    # rebuilt with the state-only --shortest-frac preselect (no wasted video encode)
    # to race the old encode-all path. Same episodes, same crops.
    TrainConfig(
        name="pi05_siemens_simple_d405_v13short25ccf_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
            repo_id="siemens_simple_d405_v13short25ccf",
            default_prompt="Pack one transparent bag into the cardboard box and flatten the bag.",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 1981,  # converted count (fast preselect path; 99% same set as non-f variants)
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    TrainConfig(
        name="pi05_siemens_simple_d405_v13short25tcf_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
            repo_id="siemens_simple_d405_v13short25tcf",
            default_prompt="Pack one transparent bag into the cardboard box and flatten the bag.",
            base_config=DataConfig(prompt_from_task=True),
            val_frac=10 / 1980,  # converted count (fast preselect path; 99% same set as non-f variants)
            val_seed=0,
        ),
        batch_size=128,
        fsdp_devices=2,
    ...
    # v13short25cc_ft13 (2026-09-11): FINE-TUNE of the v13dj_recent 20k checkpoint on
    # the shortest-25% center-crop dataset (siemens_simple_d405_v13short25cc, 1976 eps,
    # trimmed 15.3-31.6s, incl the 11 QA-rescued sub-20s packs). Same lineage as v13
    # (center-crop ALL cams, leader actions, driver joint order, packing prompt), so
    # the warm-start is transform-consistent. Hypothesis: concentrating on the fastest
    # quartile of demos sharpens efficient behavior. Fresh cosine 20k; keep-period 5k
    # snapshots (5k/10k) double as shorter-fine-tune candidates for rollout eval.
    # SERVING identical to v12/v13: center-crop the wrists too.
    TrainConfig(
        name="pi05_siemens_simple_d405_v13short25cc_ft13_bs128",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=30),
        data=LeRobotYamRormDataConfig(
            repo_id="siemens_simple_d405_v13short25cc",
            default_prompt="Pack one transparent bag into the cardboard box and flatten the bag.",
        ...
        fsdp_devices=2,
        num_workers=8,
        # Warm-start from the v13dj_recent final checkpoint on NFS.
        weight_loader=weight_loaders.CheckpointWeightLoader(
