...
def _build_run_script() -> str:
    ...
    lines += [
        ...
        '# WANDB_MODE_ARG holds an env assignment ("WANDB_MODE=disabled ") when no',
        '# wandb key exists. It must be exported, not prefix-expanded: the shell',
        '# does not re-parse an expansion as an assignment-prefix, so',
        '# ${WANDB_MODE_ARG}uv would try to exec a command named',
        '# "WANDB_MODE=disabled" and die with 127.',
        'if [ -n "$WANDB_MODE_ARG" ]; then export WANDB_MODE=disabled; fi',
